...
8. [条件付き Bean と AutoConfiguration](#条件付き-bean-と-autoconfiguration)
9. [外部化設定 (Externalized Configuration)](#外部化設定-externalized-configuration)
10. [型安全な設定 @ConfigurationProperties](#型安全な設定-configurationproperties)
11. [Bean ライフサイクル](#bean-ライフサイクル)
12. [AOP とプロキシ](#aop-とプロキシ)
...
- 言語: Java 21
- 実行形式: 実行可能 JAR（`backend-0.0.1-SNAPSHOT.jar`）

---

## 実行環境のレイヤー

`java -jar backend.jar` でアプリが動くとき、どのレイヤーが重なっているのかを整理します。
...
flowchart TB
    ...
    JAR["実行可能 JAR<br/><small>backend-0.0.1-SNAPSHOT.jar</small>"]:::runtime
...
sequenceDiagram
    ...
    Ctx->>Auto: @EnableAutoConfiguration 実行（クラスパスを走査）
...
### 図の補足: `@EnableAutoConfiguration` とコールバック

#### `@EnableAutoConfiguration` の仕組み

「クラスパスを走査」と聞くと全ファイルを舐めているように感じますが、実際は**あらかじめ用意された候補クラスの条件判定**です。ポイントは **Starter が「条件判定の対象クラス」を classpath に持ち込んでくれる**ことにあります。

1. **候補リストの読み込み**: `spring-boot-autoconfigure.jar` 内の `META-INF/spring/...AutoConfiguration.imports` に自動設定クラスの一覧が列挙されており、起動時にこのリストを読んで候補を決めます。
2. **条件判定**: 各候補には `@ConditionalOnClass(DispatcherServlet.class)` のように **「この目印クラスが classpath に存在すれば有効」** という条件が付いています。存在確認はクラスローダーへの問い合わせだけで、ソース走査は発生しません。
3. **Starter の役割**: Starter は `spring-webmvc.jar` のような**実ライブラリを classpath に持ち込む**ことで、条件アノテーションが検知する目印クラス（例: `DispatcherServlet`）を供給します。これで対応する自動設定が有効化されます。
...
結果として、`pom.xml` に `spring-boot-starter-web` を 1 行書くだけで「目印クラスの提供 → 条件成立 → Bean 登録」が連鎖し、Web アプリとして動く状態が整います。
...
「後出し」であるため `@ConditionalOnMissingBean` と組み合わせ、「ユーザーが自分で Bean を定義していれば自動設定はスキップ」という **ユーザー優先** の挙動を実現できます。そのためライブラリで配布する設定に使われる。
...
## 条件付き Bean と AutoConfiguration

AutoConfiguration は「条件に合ったときだけ Bean を登録する」仕組みで、その「条件」を表現するのが `@Conditional*` アノテーションです。
...
AutoConfiguration は裏でこれらを駆使して、**「H2 がクラスパスにあれば H2 用の DataSource を、MySQL がクラスパスにあれば MySQL 用を」**といった自動判定をしています。また、**あなたが自前で `@Bean` を定義すれば、AutoConfiguration 側は `@ConditionalOnMissingBean` で身を引く**ため、上書きできるようになっています。
