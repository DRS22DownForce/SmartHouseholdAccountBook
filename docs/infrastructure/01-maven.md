...
```xml
<groupId>com.smarthouseholdaccountbook</groupId>
...
ビルドすると、実行可能 JAR は `target/app.jar` に出力されます。`pom.xml` の `<build>` 内で `<finalName>app</finalName>` としているため、成果物のファイル名を Docker の `COPY` などと揃えやすくしています。`finalName` を書かない場合の Maven の既定では、`artifactId` と `version` をつなげた `backend-0.0.1-SNAPSHOT.jar` のような名前になります。
...
このプロジェクトは Java 25 を前提にしています。ローカルで開発する場合も、Docker 内でビルドする場合も、Java 25 にそろえるのが安全です。
...
<plugin>
    ...
    <configuration>
        <mainClass>com.smarthouseholdaccountbook.backend.BackendApplication</mainClass>
    </configuration>
</plugin>
```
