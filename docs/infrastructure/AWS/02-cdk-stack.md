...
EC2 の `bootstrap.sh` が起動時にこれらを読み、`.env` や Nginx 設定を組み立てます。
...
起動時は **User Data** が bootstrap zip を S3 から取得し `bootstrap.sh` を実行します（詳細は後述）。
...
| Bootstrap Asset（S3）読み取り | 初回起動時の zip ダウンロード |
...
userData.addCommands(
        ...
        // 環境変数（CDK がスタックから注入）
        "PROJECT_NAME=...", "AWS_REGION=...", "ECR_REPO_URI=...",
        "APP_SECRET_ARN=...", "BOOTSTRAP_ASSET_URL=...",
        // bootstrap zip の取得と実行
        "dnf install -y aws-cli unzip",
        "aws s3 cp \"$BOOTSTRAP_ASSET_URL\" /tmp/bootstrap.zip ...",
...
| `/var/log/smart-household-bootstrap.log` | 起動ログの保存先 |
| `ECR_REPO_URI` | pull する Backend イメージの ECR URL |
| `APP_SECRET_ARN` | Secrets Manager の ARN（bootstrap が DB パスワード等を取得） |
| `BOOTSTRAP_ASSET_URL` | bootstrap zip の S3 URL（下記 Asset） |
| `bootstrap.sh` | Docker / Nginx / certbot など本格的なセットアップ |

User Data は **zip 取得と起動だけ**に留め、詳細なセットアップは `bootstrap.sh` に任せています。頻繁に変わる処理を bootstrap 側に寄せ、User Data の変更（＝ EC2 再作成）を減らす設計です。
...
sequenceDiagram
    ...
    CDK->>S3: assets/ec2-bootstrap をアップロード
    CDK->>EC2: User Data で S3 URL を渡す
    EC2->>S3: aws s3 cp bootstrap.zip
    EC2->>Bootstrap: 展開して実行
```

1. CDK が `infra/assets/ec2-bootstrap/` を S3 にアップロード
2. User Data が IAM ロール経由で `aws s3 cp` し zip を取得
3. `/opt/smart-household/bootstrap/bootstrap.sh` を実行（Nginx 設定・Docker Compose 起動など）
...
- EC2 初回起動は User Data → S3 から bootstrap zip → `bootstrap.sh` の順です。
