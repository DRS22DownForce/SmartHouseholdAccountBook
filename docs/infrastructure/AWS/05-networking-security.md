...
`domainName` がゾーン apex（`example.com`）のとき、A レコードの `recordName` は省略されます。サブドメイン（`app.example.com`）のときは、ラベル部分（`app`）だけが record 名になります。
...
2. `setup_https` — DNS 確認後、`certbot --nginx` で証明書取得
