...
`domainName` がゾーン apex（`example.com`）のとき、A レコードの `recordName` は省略されます。サブドメイン（`app.example.com`）のときは、ラベル部分（`app`）だけが record 名になります。いずれも **`domainName` 向けの A レコード 1 本のみ**です。
...
### Let's Encrypt と certbot とは

| 用語 | 役割 |
|------|------|
| **Let's Encrypt** | 無料で HTTPS 証明書を発行する認証局（CA）。有効期限は約 90 日 |
| **certbot** | Let's Encrypt と連携し、証明書の取得・更新と Nginx 設定の更新を行う CLI ツール |

Let's Encrypt は「そのドメインを本当に管理しているか」を **チャレンジ** で確認してから証明書を発行します。このプロジェクトは **HTTP-01** を使い、Let's Encrypt が `http://{domain}/.well-known/acme-challenge/...` にアクセスできることを確認します（ポート 80 が必要な理由のひとつです）。

### Nginx：初回は HTTP、certbot が HTTPS を設定

Nginx のテンプレート（`infra/assets/ec2-bootstrap/nginx/smart-household.conf`）は **ポート 80（HTTP）だけ** を listen し、SSL 証明書のパスは書きません。bootstrap は次の順で動きます。

1. `configure_nginx` — テンプレートを配置して Nginx 起動（HTTP のみ。アプリへのプロキシはここで有効）
2. `setup_https` — DNS 確認後、`certbot --nginx -d {domainName}` で証明書取得

certbot は証明書を `/etc/letsencrypt/live/.../` に保存し、`/etc/nginx/conf.d/smart-household.conf` を自動編集して **443（HTTPS）の server ブロック** と **HTTP → HTTPS リダイレクト**（`--redirect`）を追加します。HTTPS 部分を手書きせず、証明書取得後に certbot に任せる設計です。

初回 bootstrap から HTTPS 有効化までの流れは次のとおりです。
...
sequenceDiagram
  ...
  Note over BS,Nginx: ① configure_nginx
    BS->>Nginx: HTTP テンプレートを配置して起動
    Note over Nginx: listen 80 のみ（SSL 未設定）

  Note over BS,DNS: ② wait_for_dns（最大約5分）
    loop dig で Elastic IP と一致するまで待機
        BS->>DNS: dig {domain} A
        DNS-->>BS: 解決された IP
    end

  Note over BS,LE: ③ setup_https（certbot --nginx --redirect）
    BS->>CB: 証明書取得を実行
    CB->>LE: 証明書発行をリクエスト
    LE-->>CB: HTTP-01 チャレンジを返す
    CB->>Nginx: /.well-known/acme-challenge 用の設定を追加
    CB->>LE: チャレンジ検証を依頼
    LE->>Nginx: GET http://{domain}/.well-known/acme-challenge/{token}
    Nginx-->>LE: チャレンジ応答
    LE-->>CB: ドメイン確認 OK・証明書を発行
    CB->>CB: 証明書を /etc/letsencrypt/live/.../ に保存
    CB->>Nginx: listen 443 + ssl_certificate を設定
    CB->>Nginx: HTTP → HTTPS リダイレクトを設定
    CB-->>BS: 完了
    BS->>BS: certbot-renew.timer を有効化
```
...
CDK（`SmartHouseholdStack`）が `domainName` から CORS 許可 Origin を **`https://{domainName}` 1 件**組み立て、SSM `/domain/cors-allowed-origins` に保存します。Spring Boot がフロントからの API 呼び出しを許可するために必要です。
