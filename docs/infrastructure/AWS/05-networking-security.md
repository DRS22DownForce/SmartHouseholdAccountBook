...
| 低〜中機密 | SSM Parameter Store | Cognito Client ID、ドメイン名 |
...
`init-secrets.sh` は既定で `https://{domain}` と `https://www.{domain}` を `CORS_ALLOWED_ORIGINS` に入れます。Spring Boot がフロントからの API 呼び出しを許可するために必要です。
...
- DB パスワードは **Secrets Manager**、Cognito ID は **SSM** に分けて保管します。
