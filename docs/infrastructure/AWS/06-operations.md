...
| ログ | 内容 |
|------|------|
| `/var/log/smart-household-user-data.log` | EC2 起動時の User Data（aws-cli インストール等） |
| `/var/log/smart-household-bootstrap.log` | `deploy-app.sh` 経由の bootstrap 出力 |
...
- 一時的に課金を抑えるなら **`pause.sh`**（EC2 停止）。再開は **`resume.sh`**。完全に止めるなら **`destroy.sh`**。
