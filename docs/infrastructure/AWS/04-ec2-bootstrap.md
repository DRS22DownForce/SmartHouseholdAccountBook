...
**ブートストラップ**とは、EC2 が初めて起動したとき（または修復が必要なとき）に、OS の準備からアプリ起動までを自動で行う処理です。
...
flowchart TB
    ...
    UD --> BS --> PK --> GIT --> ENV --> NGX --> TLS --> DC --> FE
```

User Data は短いラッパーに留め、本体は `/opt/smart-household/bootstrap/bootstrap.sh` です。
...
|----|------|
| （未設定） | フル bootstrap |
| `refresh-env-only` | Secrets の最新値で `.env` を書き直し、Compose 再起動 |
| `frontend-only` | Next.js のビルドと systemd 起動だけ |

`deploy-app.sh` からの更新では、多くの場合 `refresh-env-only` 相当の処理（`restart_backend_only`）が使われます。
...
1. `dig` でドメインが EC2 の Elastic IP を向いているか確認（最大約 5 分待機）
2. `certbot --nginx` で証明書取得
3. HTTP → HTTPS リダイレクトを設定
4. `certbot-renew.timer` で自動更新

ルートドメイン（`example.com`）のときは `www.example.com` も証明書に含めます。
...
`deploy-app.sh` が SSM 経由で呼ぶ EC2 側のエントリポイントです。

```mermaid
flowchart TD
    Start["remote-app-deploy.sh"]
    DL["bootstrap zip 修復（必要なら）"]
    Check{"bootstrap 完了?"}
    Full["bootstrap.sh フル実行"]
    Refresh[".env 更新 + docker pull"]
    FE{"Frontend 要セットアップ?"}
    FEOnly["frontend-only bootstrap"]

    Start --> DL --> Check
    Check -->|No| Full
    Check -->|Yes| Refresh --> FE
    FE -->|Yes| FEOnly
    FE -->|No| Done["完了"]
    FEOnly --> Done
    Full --> Done
...
`needs_full_bootstrap` が true になる条件の例:

- `.bootstrap-complete` が無い
- `docker` コマンドが無い
- `.env` が無い
- `docker-compose.single-host.aws.yaml` が無い

未セットアップの EC2 に対して `deploy-app.sh` を実行しても、自動的にフル bootstrap にフォールバックします。
...
- EC2 起動時は `bootstrap.sh` が OS 準備からアプリ起動までを自動化します。
