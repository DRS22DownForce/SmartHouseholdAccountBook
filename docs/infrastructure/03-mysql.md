...
| ファイル | 見るポイント |
|----------|--------------|
| `docker-compose.dev.yaml` | MySQL だけをローカル開発用に起動する設定 |
| `docker-compose.single-host.yaml` | MySQL とバックエンドを同じ Docker ネットワークで起動する設定 |
| `docker-compose.single-host.local.yaml` | ローカル用のポート公開と SQL ログ設定 |
| `docker-compose.single-host.prod.yaml` | 本番寄せの SQL ログ抑制設定 |
...
初心者向けに言うと、`docker-compose*.yaml` は「MySQL をどう起動するか」、`application.properties` は「Spring Boot が MySQL へどう接続するか」、`V1__initial_schema.sql` は「MySQL の中にどんな表を作るか」です。
...
|----------|-------------------|----------------|-------------------|
| `docker-compose.dev.yaml` | ローカルPC / IDE | `localhost:3306` | `localhost` |
| `docker-compose.single-host.yaml` | Docker コンテナ内 | Compose サービス名 | `mysql` |
...
`docker-compose.single-host.yaml` では、バックエンドコンテナに次の環境変数を渡します。
...
`docker-compose.dev.yaml` では MySQL だけを起動します。
